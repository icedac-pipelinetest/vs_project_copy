...
#pragma comment(lib, "rpcrt4.lib")  // UuidCreate - Minimum supported OS Win 2000
 ...
 3.1 if both of old, new name ended with 'Test', take the prefix
 %old_trimmed_trailing_test% -> %new_trimmed_trailing_test%

 4. give a new UUID if founded
...
public:
	...
	void BuildRule() {
		// build replace rule
		m_rulesReplace.emplace_back(Rule({ m_srcName, m_dstName }));

		auto uppercasedSrcName = std::wstring(m_srcName);
		for (auto& c : uppercasedSrcName) c = towupper(c);
		auto uppercaseddstName = std::wstring(m_dstName);
		for (auto& c : uppercaseddstName) c = towupper(c);

		m_rulesReplace.emplace_back(Rule({ FormatStringW(L"_%s_", uppercasedSrcName.c_str()).AsStr(), FormatStringW(L"_%s_", uppercaseddstName.c_str()).AsStr() }));

		// if both of src, dst name ended with 'Test', take the prefix name also for replace rule
		const auto* s = ReplaceName(m_srcName, L"Test", L"");
		if (!s) return;
		std::wstring srcTestTrimmed = s;
		s = ReplaceName(m_dstName, L"Test", L"");
		if (!s) return;
		std::wstring dstTestTrimmed = s;

		m_rulesReplace.emplace_back(Rule({ srcTestTrimmed, dstTestTrimmed }));
	}
...
public:
	...
	// 
	static const wchar_t* ReplaceName(const wchar_t* line, const wchar_t* from, const wchar_t* to)
	{
		wchar_t* s = nullptr;
	...
	static void FindFile(const _TCHAR* dir, const _TCHAR* fileFilter, std::function< void(const _TCHAR*) > func)
	{
	...
	static void RenameFiles(const _TCHAR* srcDir, const _TCHAR* src, const _TCHAR* dst)
	{
	...
	static bool DoTextDialogYesOrNo(const _TCHAR* msg)
	{
	...
	static void CopyDir(const _TCHAR* src, const _TCHAR* dst)
	{
	...
	void Run()
	{
		...
		for (const auto& rule : m_rulesReplace) {
			RenameFiles(m_dstName, rule.from.c_str(), rule.to.c_str());
		}
	...
	const wchar_t* DoLine(const wchar_t* line)
	{
		...
		// replace with rules
		for (const auto& rule : m_rulesReplace) {
			while (const wchar_t* re = ReplaceName(buf, rule.from.c_str(), rule.to.c_str()))
			{
				changed = true;
...
private:
	...
	std::vector< Rule > m_rulesReplace;
};
